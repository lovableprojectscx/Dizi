...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!urlObj.hostname.includes("facebook.com")) {
      return res.status(400).json({ error: "El enlace debe ser de una página de Facebook válida (facebook.com)." });
    }
    ...
    if (!fbRes.ok) {
      ...
      return res.status(400).json({ error: "No se pudo acceder a la página de Facebook. Asegúrate de que la página sea pública." });
    }
    ...
    if (title) {
      ...
      title = title.replace(/\s*\|\s*Facebook\s*/gi, "").replace(/\s*-\s*Home\s*/gi, "").trim();
    } else {
      title = "Mi Tienda";
    }
    ...
    if (!ogImageMatch) {
      ...
      return res.status(404).json({ error: "No se pudo extraer la foto de perfil. Asegúrate de que el enlace sea correcto y la página sea pública." });
    }
    ...
    if (!imgRes.ok) {
      ...
      return res.status(400).json({ error: "No se pudo descargar la foto de perfil desde Facebook." });
    }
