...
const PLAN_LEVELS: Record<PlanId, number> = {
  semilla: 0, emprendedor: 1, pro: 3, ilimitado: 3,
};
...
1. `types.ts`: cambiar `PlanId` de union cerrado a `string`, y convertir `PLANS` de constante a **cache cargada desde DB** (con los 4 default como *fallback* de seguridad si la red falla → evita el crash del § 2.2).
...
*Documento de evaluación. No ejecuta cambios. Al aprobar, se redacta el plan de implementación detallado por fase.*
