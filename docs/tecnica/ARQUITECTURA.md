...
3. `upsertProduct()` detecta la imagen en base64 y la sube a Supabase Storage mediante `uploadBase64ToStorage()` bajo la ruta `[storeId]/products/[productId].webp`
4. Se guarda la URL pública del objeto de Storage en la columna `image` de la tabla en Supabase (con fallback a base64 solo si falla la subida)
...
_Fin del documento. Mantener actualizado ante cambios estructurales._
