...
BEGIN
  ...
  -- 2. Calcular días transcurridos desde el vencimiento del plan
  ...
  ELSE
    days_expired := -1; -- Negativo indica plan aún vigente o sin expiración
  END IF;

  -- 3. Determinar el plan efectivo (Semilla si venció hace más del período de gracia de 3 días)
  ...
  -- 4. Determinar el límite de productos del plan efectivo
  IF store_plan = 'semilla' THEN
    effective_limit := 7;
  ELSE
    effective_limit := 999999; -- Sin límite práctico
  END IF;

  -- 5. Determinar el modelo efectivo (Semilla usa minimalista. Premium vencido cambia a minimalista después de 15 días de gracia)
  IF store_row.plan = 'semilla' THEN
  ...
  -- 6. Construir el resultado en formato JSONB aplicando las restricciones correspondientes
  SELECT jsonb_build_object(
    ...
    'plan', store_plan, -- Plan real o degradado a Semilla
    'model', effective_model, -- Modelo real o degradado a minimalista
    'active', store_row.active,
    'is_published', store_row.is_published,
    'created_at', store_row.created_at,
    'whatsapp_clicks', store_row.whatsapp_clicks,
    ...
    -- Forzar valores por defecto/nulos para características premium si el plan es Semilla
    ...
    -- Listar productos activos (limitados a nivel de servidor por el límite del plan y sin la columna image)
    'products', COALESCE((
