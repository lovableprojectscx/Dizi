-- Migration: Add sort_order to products and update get_public_store RPC

-- 1. Añadir la columna sort_order a la tabla products si no existe
ALTER TABLE products ADD COLUMN IF NOT EXISTS sort_order integer DEFAULT 0 NOT NULL;

-- 2. Crear un índice compuesto para acelerar consultas y ordenamientos por tienda
CREATE INDEX IF NOT EXISTS idx_products_store_sort 
ON products(store_id, sort_order ASC, created_at DESC);

-- 3. Inicializar secuencialmente el sort_order para los productos existentes por tienda
WITH ordenados AS (
  SELECT id, ROW_NUMBER() OVER (PARTITION BY store_id ORDER BY created_at ASC) as row_num
  FROM products
)
UPDATE products p
SET sort_order = o.row_num
FROM ordenados o
WHERE p.id = o.id;

-- 4. Actualizar la función RPC get_public_store para incluir sort_order y ordenar por él
