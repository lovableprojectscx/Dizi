-- Migración: Optimización de Egress y Paginación Lazy Loading para Catálogos Públicos
-- Fecha: 2026-08-25

-- 1. Eliminar versiones anteriores ambiguas
DROP FUNCTION IF EXISTS get_public_store(text);
DROP FUNCTION IF EXISTS get_public_store_products(text, integer, integer, text);
DROP FUNCTION IF EXISTS get_public_store_products(text, integer, integer, text, text);

-- 2. Actualizar get_public_store con soporte de page_limit, page_offset y total_products_count
CREATE OR REPLACE FUNCTION get_public_store(
  store_slug text,
...
BEGIN
  ...
  -- Contar el total de productos visibles para este store (hasta el límite de su plan)
  SELECT LEAST(count(*), effective_limit) INTO total_prod_count
  FROM products
  WHERE store_id = store_row.id AND visible = true;

  -- Calcular el límite real para este lote
...
-- 3. Crear RPC get_public_store_products para carga paginada bajo demanda
CREATE OR REPLACE FUNCTION get_public_store_products(
  p_store_slug text,
  p_page_offset int DEFAULT 0,
  p_page_limit int DEFAULT 24,
...
DECLARE
  ...
  result jsonb;
BEGIN
  SELECT id, plan, plan_expires_at INTO v_store_row FROM stores WHERE slug = p_store_slug AND active = true LIMIT 1;
  IF v_store_row IS NULL THEN
    RETURN '[]'::jsonb;
  END IF;

  IF v_store_row.plan = 'semilla' THEN
    v_effective_limit := 20;
  ELSIF v_store_row.plan = 'emprendedor' THEN
    v_effective_limit := 100;
  ELSIF v_store_row.plan = 'pro' THEN
    v_effective_limit := 300;
  ELSE
    v_effective_limit := 1000;
  END IF;
  ...
  FROM (
    ...
    WHERE store_id = v_store_row.id 
      ...
      AND (p_search_query IS NULL OR p_search_query = '' OR products.name ILIKE '%' || p_search_query || '%')
    ORDER BY sort_order ASC NULLS LAST, created_at DESC
    LIMIT LEAST(COALESCE(p_page_limit, 24), v_effective_limit)
