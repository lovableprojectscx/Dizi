-- Migration: Add bio_show_catalog_button to stores and expose it in get_public_store
-- Cuando es NULL se mantiene el comportamiento histórico (el botón solo aparece con tipografía serif).

ALTER TABLE public.stores ADD COLUMN IF NOT EXISTS bio_show_catalog_button boolean DEFAULT NULL;
...
BEGIN
  ...
  IF store_plan = 'semilla' THEN
    effective_limit := 20; -- ACTUALIZADO DE 7 A 20
  ELSE
    ...
    'quick_links', (
      CASE 
        WHEN store_plan = 'semilla' THEN 
          COALESCE((SELECT jsonb_agg(value) FROM (SELECT value FROM jsonb_array_elements(COALESCE(store_row.quick_links, '[]'::jsonb)) LIMIT 3) t), '[]'::jsonb) -- ACTUALIZADO DE 5 A 3
        ELSE 
