...
function RegisterPage() {
  ...
  const ALL_MODELS = [
    { id: "elite",        name: "Elite",            desc: "Portada con Banner cinematografico", niches: ["general","ropa","tech"],         p: { bg: "#ffffff", card: "#ffffff", accent: "#1e1e1e", dark: false } },
    ...
    { id: "eco",          name: "Eco Nature",       desc: "Organico y sustentable",            niches: ["servicios","general"],           p: { bg: "#f0fdf4", card: "#ffffff", accent: "#16a34a", dark: false } },
    ...
    { id: "clasico",      name: "Clasico Creme",    desc: "Calidez atemporal",                 niches: ["general","servicios"],          p: { bg: "#fdfaf5", card: "#fef9ef", accent: "#92400e", dark: false } },
    ...
    { id: "terroso",      name: "Terroso Artesanal",desc: "Tonos tierra organicos",            niches: ["comida","servicios"],           p: { bg: "#fef3c7", card: "#fde68a", accent: "#92400e", dark: false } },
    { id: "marina",       name: "Marina Azul",      desc: "Fresco maritimo y limpio",          niches: ["servicios","general"],          p: { bg: "#f0f9ff", card: "#e0f2fe", accent: "#0284c7", dark: false } },
    ...
    { id: "sunset",       name: "Sunset Warm",      desc: "Atardecer dorado y calido",         niches: ["comida","general"],             p: { bg: "#fff7ed", card: "#ffedd5", accent: "#ea580c", dark: false } },
    ...
    { id: "urban",        name: "Urban Street",     desc: "Urbano y dinamico",                 niches: ["tech","ropa"],                  p: { bg: "#18181b", card: "#27272a", accent: "#facc15", dark: true  } },
  ];

  const models = isPremium
    ? ALL_MODELS.map(m => ({ ...m, locked: false, recommended: m.niches.includes(selectedNiche) }))
    : ALL_MODELS.map((m, idx) => ({ ...m, locked: idx > 0, recommended: false }));
    ...
    if (step < 3) {
      ...
      try {
        ...
        const { data: authData, error: authError } = await supabase.auth.signUp({
          ...
          password: password,
          options: {
            data: { full_name: fullName, role: "store_owner" }
          }
        });
    ...
    <div className="min-h-screen bg-muted/30 flex flex-col" translate="no">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-lg bg-background rounded-3xl border shadow-xl p-8 relative overflow-hidden">
          ...
          {inviteLoading && (
            <div className="absolute top-0 inset-x-0 bg-muted text-muted-foreground text-xs font-medium text-center py-1.5 px-4 tracking-wide z-10">
          ...
          {!inviteLoading && invitePlan && (
            <div className="absolute top-0 inset-x-0 bg-primary text-primary-foreground text-xs font-bold text-center py-1.5 px-4 tracking-wide z-10">
              Invitacion Especial Activa: Plan {plan.toUpperCase()}
            </div>
          )}
          {!inviteLoading && inviteToken && !invitePlan && (
            <div className="absolute top-0 inset-x-0 bg-destructive text-destructive-foreground text-xs font-bold text-center py-1.5 px-4 tracking-wide z-10">
              Enlace de invitacion invalido, expirado o ya usado — se aplicara plan Semilla
            </div>
          )}

          <div className={`text-center mb-8 relative ${invitePlan || (inviteToken && !inviteLoading) ? "pt-4" : ""}`}>
            ...
            <p className="text-muted-foreground text-sm mt-2">
              Paso {step} de 3: {step === 1 ? "Datos de tu cuenta" : step === 2 ? "Datos de tu negocio" : "Personaliza tu Catalogo"}
            </p>
          </div>
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="flex h-12 w-full rounded-xl border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary"
                    placeholder="Ej. Novedades Maria"
                    required
                  />
                </div>
                <div className="space-y-2">
                ...
                {isPremium && (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Selecciona tu nicho y te recomendaremos el mejor diseno para tu tipo de negocio.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button type="submit" className="flex-1 h-12 text-base" disabled={loading}>
                    {loading ? (
                      ...
                      <>
                        Lanzar Catalogo <Rocket className="ml-2 w-4 h-4" />
                      </>
                    )}
                  </Button>
                </div>
