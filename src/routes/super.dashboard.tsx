        ...
        <div className="space-y-1">
          {payload.map((p: any, idx: number) => (
            <p key={idx} className="font-semibold flex items-center justify-between gap-4">
              ...
              <span className="font-mono text-foreground font-bold">{p.value.toLocaleString()}</span>
            </p>
          ))}
        </div>
      </div>
...
function SuperDashboard() {
  ...
  const premiumRatioPercent = totalStores > 0 ? Math.round((premiumStores / activeStores) * 100) : 0;
    ...
    <div className="space-y-6 max-w-7xl animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          trend={{ value: `~${totalStores > 0 ? Math.round(totalClicks / totalStores) : 0} c/tienda`, positive: true }}
        />
        <MetricCard
            ...
            {trendData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="mes" tickLine={false} axisLine={false} className="text-[10px] text-muted-foreground" />
                  <YAxis tickLine={false} axisLine={false} className="text-[10px] text-muted-foreground" />
                  <RechartsTooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="Nuevos Registros" stroke="#4f46e5" strokeWidth={2} fillOpacity={1} fill="url(#colorCount)" />
                  <Area type="monotone" dataKey="Total Acumulado" stroke="#06b6d4" strokeWidth={2} fillOpacity={1} fill="url(#colorCum)" />
            ...
            {nicheData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={nicheData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" opacity={0.5} />
                  <XAxis dataKey="niche" tickLine={false} axisLine={false} className="text-[10px] text-muted-foreground" />
                  <YAxis tickLine={false} axisLine={false} className="text-[10px] text-muted-foreground" />
                  <RechartsTooltip content={<CustomTooltip />} />
              ...
              <>
                <div className="w-full sm:w-1/2 h-56 relative flex items-center justify-center">
                  ...
                  <div className="absolute text-center flex flex-col">
                    ...
                    <span className="text-[9px] text-muted-foreground uppercase font-bold tracking-wider">Activas</span>
                  </div>
                </div>
                <div className="w-full sm:w-1/2 flex flex-col gap-2.5 text-xs px-2">
                  {planData.map((item) => (
                    <div key={item.name} className="flex items-center justify-between border-b pb-1">
                      <span className="flex items-center gap-2 font-medium text-muted-foreground">
                        <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: item.fill }} />
                        {item.name}
                      </span>
                      <span className="font-bold font-mono">
                        {item.value} <span className="text-muted-foreground/60 font-normal">({Math.round((item.value / totalStores) * 100)}%)</span>
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <Card className="border-border/50 shadow-sm hover:shadow-md transition-shadow duration-300">
          ...
          <CardContent className="h-80 pt-4">
            {topClicksData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topClicksData} layout="vertical" margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e2e8f0" opacity={0.5} />
                  <XAxis type="number" tickLine={false} axisLine={false} className="text-[10px] text-muted-foreground" />
                  <YAxis dataKey="name" type="category" tickLine={false} axisLine={false} className="text-[11px] font-semibold text-foreground" width={90} />
                  <RechartsTooltip content={<CustomTooltip />} />
                  ...
                  {latestStores.map((s) => (
                    <tr key={s.id} className="hover:bg-muted/10 transition-colors">
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          {s.logo ? (
                            <img src={s.logo} alt="" className="h-6 w-6 rounded-full object-cover shrink-0 border" />
                          ) : (
                          ...
                          <div className="min-w-0">
                            ...
                            <span className="text-[10px] text-muted-foreground uppercase">{nicheLabels[s.niche || "general"]}</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-3">
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y">
                    ...
                    <tr key={s.id} className="hover:bg-muted/10 transition-colors">
                      ...
                      <td className="p-3">
                        {!s.active ? (
                          ...
                          <Badge variant="outline" className="text-[10px] py-0 px-1.5 text-rose-600 border-rose-200">
                            Expirado
                          </Badge>
                        )}
                      </td>
                  ...
                  {churnAlerts.length === 0 && (
                    <tr>
                      <td colSpan={4} className="text-center text-muted-foreground py-6 text-xs italic">
        ...
        <Card className="border-border/50 shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col justify-between">
          ...
          <CardContent className="space-y-4">
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
              ...
              <div>
                <span className="text-xs text-muted-foreground block">Tiendas con Libro Activo</span>
                <span className="text-2xl font-black text-blue-800">{storesWithLibro}</span>
              </div>
            </div>
            
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              Cumplimiento con las normativas de protección al consumidor del INDECOPI en Perú.
              Los comercios activos que superen su plan gratuito semilla pueden activar este módulo en sus configuraciones.
            </p>
          </CardContent>
        </Card>
